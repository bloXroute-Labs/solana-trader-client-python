...
class Memo:
    ...
    @staticmethod
    def add_memo(instructions, memo_content: str, blockhash, owner, private_keys):
        memo = Memo.create_trader_api_memo_instruction(memo_content)

        instructions.append(memo)

        txn_bytes, err = Memo.build_fully_signed_txn(blockhash, owner, instructions, private_keys)
        if err is not None:
            return ""
    ...
    @staticmethod
    def build_fully_signed_txn(recent_block_hash, owner, instruction: solana_transaction.TransactionInstruction, private_keys: Keypair):
