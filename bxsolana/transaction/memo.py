...
def create_trader_api_memo_instruction(msg):
    ...
    instruction = solana_transaction.TransactionInstruction(keys=[], program_id=TraderAPIMemoProgram,
                                                            data=data)
...
def add_memo(instructions: list[solana_transaction.TransactionInstruction], memo_content: str, blockhash, owner, *private_keys: Keypair):
    ...
    txn_bytes = build_fully_signed_txn(blockhash, owner, instructions, *private_keys)
...
def add_memo_to_serialized_txn(tx_base64: str, memo_content: str, owner, *private_keys: Keypair):
    ...
    for cmpInst in tx.compile_message().instructions:
        accounts = []
        for key in cmpInst.accounts:
            inst = tx.compile_message().account_keys[key]
            accounts.append(inst)

        new_tx.instructions.append(
            solana_transaction.TransactionInstruction(
                keys=accounts, program_id=tx.compile_message().account_keys[cmpInst.program_id_index], data=cmpInst.data))
